import { defaults, debounce, isMissingOption, interpolateUrl, getPath, setPath, pushPath, defer, sanitizeLogValue, redactUrlCredentials } from './utils.js'
...
// return base +-25% randomised timeout (ms)
...
class I18NextLocizeBackend {
  ...
  init (services, options = {}, allOptions = {}, callback) {
    ...
    if (this.options.noCache && this.options.cdnType !== 'standard') {
      console.warn(
        `The 'noCache' option is not available for 'cdnType' '${this.options.cdnType}'!`
      )
    }
  ...
  read (language, namespace, callback) {
    ...
    if (this.warnedLanguages && this.warnedLanguages.indexOf(language) > -1) {
      const err = new Error(`Will not continue to load language "${language}" since it is not available in Locize project ${this.options.projectId}!`)
      if (logger) logger.error(err.message)
      if (callback) callback(err)
      return
    }
    ...
    this.loadUrl(options, url, (err, ret, info) => {
      ...
      if (!resourceNotExisting) {
        this.hasResourcesForLng ||= {}
        this.hasResourcesForLng[language] = true
      }
      if (resourceNotExisting && (!this.hasResourcesForLng || !this.hasResourcesForLng[language])) {
        setTimeout(() => {
      ...
      if (!this.somethingLoaded) {
        if (resourceNotExisting) {
          setTimeout(() => this.checkIfProjectExists(), randomizeTimeout(this.options.checkForProjectTimeout))
        } else {
          this.somethingLoaded = true
        }
      }
  ...
  loadUrl (options, url, payload, callback) {
    ...
    // stripped) and log-sanitised (CR/LF/control chars replaced): loadPath /
    // privatePath are user-configurable, so basic-auth credentials embedded
    // there would otherwise reach i18next's logger and any attached error
    // reporting, and crafted values could forge log lines (CWE-117). The
    // request itself still uses the untouched `url`.
    ...
    const clb = (err, res) => {
      ...
      if (res && (res.status === 408 || res.status === 400)) { // extras for timeouts on cloudfront
        return callback('failed loading ' + safeUrl, true /* retry */, { resourceNotExisting })
      }
      if (res && ((res.status >= 500 && res.status < 600) || !res.status)) { return callback('failed loading ' + safeUrl, true /* retry */, { resourceNotExisting }) }
      if (res && res.status >= 400 && res.status < 500) { return callback('failed loading ' + safeUrl, false /* no retry */, { resourceNotExisting }) }
      if (!res && err && err.message) {
        ...
        if (isNetworkError) {
          return callback('failed loading ' + safeUrl + ': ' + sanitizeLogValue(err.message), true /* retry */, { resourceNotExisting })
        }
      }
