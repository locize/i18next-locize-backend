...
class I18NextLocizeBackend {
  ...
  loadUrl (options, url, payload, callback) {
    ...
    const clb = (err, res) => {
      ...
      if (!res && err && err.message && (err.message.indexOf('Failed to fetch') > -1 || (err.message.indexOf('failed') > -1 && err.message.indexOf('fetch') > -1))) return callback('failed loading ' + url + ': ' + err.message, true /* retry */, { resourceNotExisting })
