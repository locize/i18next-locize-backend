...
class I18NextLocizeBackend {
  ...
  init (services, options = {}, allOptions = {}, callback) {
    ...
    const cdnType = options.cdnType || (this.options || {}).cdnType
    // uncomment this on next major version
    // if (!cdnType) {
    ...
    // }
    const noCache = options.noCache || (this.options || {}).noCache
    this.services = services
    const defOpt = getDefaults(cdnType)
